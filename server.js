...
console.log('The server is running yippee!');
    ...
    */
    ...
    socket.on('join_room', (payload) => {
        ...
        if ((typeof room == 'undefined') || (room === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'client did not select room to join';
            socket.emit('join_room_response', response);
            serverLog('join_room command failed', JSON.stringify(response));
            return;
        }
    ...
    socket.on('uninvite', (payload) => {
        ...
        io.in(room).allSockets().then((sockets) => {
            ...
            /* UnInvitee is in the room */
                ...
                serverLog('uninvite invite command succeeded', JSON.stringify(response));
            }
        });
    });
    ...
    */
    ...
    socket.on('play_token', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'play_token command did not come from a registered userename';
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
        }
        ...
        /* Ensure current play is coming from the expected player */
        if (
...
/* Return true if r + dr supports playing at r and c _dc supports playing at c */
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    /* Check to make sure that there is space for a matching color to capture a token */
